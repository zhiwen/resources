 ...
 */
@Service
public class DoubanMovieSearchSpiderJob implements SpiderJob {

    private final static Logger log                 = LoggerFactory.getLogger(DoubanMovieSearchSpiderJob.class);

    private final String        doubanSubjectSearch = "http://movie.douban.com/j/search_subjects?type=movie&tag=%s&sort=time&page_limit=%s&page_start=%s";
    ...
    public List<SpiderRecordDO> preData() {
        ...
        if (CollectionUtils.isNotEmpty(spiderRecrods)) {
            List<SpiderRecordDO> tvList = spiderRecordMapper.getRecordByType(SpiderRecordDO.SpiderRecordTypeEnum.douban_tv.getValue());
            if (CollectionUtils.isNotEmpty(tvList)) {
                spiderRecrods.addAll(tvList);
            }
        ...
        JSONObject jsonObject = getData("http://movie.douban.com/j/search_tags?type=movie");
